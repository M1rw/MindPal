...
For long-story presence, the Gemini API production default is `gemini-2.5-flash-native-audio-preview-12-2025` with v1beta. The rejected `gemini-2.5-flash-live-preview` alias was not a valid Gemini API Live identifier and caused the observed code 1008 close. Native audio is now the actual default; Gemini 3.1 remains available with its v1alpha/manual cue path for compatibility.
