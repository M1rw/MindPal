...
def _service(monkeypatch: pytest.MonkeyPatch, *, now: float = 7200.0) -> dictation.DictationService:
    ...
    monkeypatch.setattr(
        dictation, "transcribe_audio", lambda audio, content_type: transcribe.Transcript("hello", "english", "groq")
    )
...
def test_provider_failure_is_a_clean_503(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def boom(audio: bytes, content_type: str) -> transcribe.Transcript:
