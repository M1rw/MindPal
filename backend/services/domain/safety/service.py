# backend/services/domain/safety/service.py
...
class SafetyService:
    ...
    def __init__(
        ...
        *,
        ...
        self.safety_dir = safety_dir or Path(__file__).resolve().parents[3] / "safety"
    ...
    async def _classify_with_llm(
        ...
        *,
        ...
        if provider_used.lower() == "offline" and not self.allow_offline_llm_classifier:
            raise SafetyError(
                "Offline LLM fallback cannot be used for safety ambiguity classification",
                code="safety_llm_offline_classifier_disabled",
            )

        parsed_payload = self._parse_llm_json(response.text)
    ...
    def _decision_from_llm_payload(
        ...
        *,
        ...
        confidence = self._bounded_float(payload.get("confidence", 0.5), default=0.5)
        category = sanitize_text(
            str(payload.get("user_visible_category") or self._default_category_for_level(level)),
            80,
        )
        rationale = sanitize_text(str(payload.get("rationale") or "llm safety classifier"), 300)
        matched_rules = self._clean_rule_ids(payload.get("matched_rules", []))
        ...
        if level == SafetyLevel.SELF_HARM_IMMINENT:
            return SafetyDecision(
                ...
                bypass_llm=True,
                response_template_id=self._imminent_template_for_locale(locale),
        ...
        return SafetyDecision(
            ...
            user_visible_category=category or self._default_category_for_level(level),
            action=action,
    ...
    def _merge_local_and_llm_decisions(
        ...
        if _SAFETY_RANK[llm_decision.level] == _SAFETY_RANK[local_decision.level]:
            merged_rules = self._unique_rule_ids(
    ...
    def _clean_rule_ids(value: Any) -> list[str]:
        ...
        return SafetyService._unique_rule_ids([sanitize_text(str(item), 120) for item in raw_items])
