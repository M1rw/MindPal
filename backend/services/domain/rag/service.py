# backend/services/domain/rag/service.py
...
class RAGService:
    ...
    async def _build_llm_plan(
        ...
        *,
        ...
        if provider_used.lower() == "offline" and not self.allow_offline_llm_planner:
            raise RAGError(
...
def _clean_terms(value: Any) -> tuple[str, ...]:
    return tuple(
        _unique_ordered(
            _clean_list(value, max_items=80, max_chars=120)
        )
    )
