...
On 2026-08-18, MindPal production accepted the secure native-audio token and completed setup, but then cleanly ended fresh calls shortly after the greeting. The session declared seven provider functions and the prompt allowed a profile lookup during the greeting. Independent native-audio preview reports describe the same post-first-response disconnect when function calling is enabled. MindPal now starts native-audio calls with **no provider function declarations**, retaining only provider-supported proactive audio and real `realtimeInput` listening presence. Current facts continue to use MindPal’s authenticated verified-evidence route, so the tool-free provider setup does not restore stale model-memory answers. [6]

This repair does not fake tool-level full duplex. It preserves native proactive listening presence, continuous microphone capture, barge-in, AI-only captions, lifecycle safety, verified fact routing, timezone context, and the conversation firewall. Native provider function calls and their claimed spoken background bridges remain disabled until a tool-enabled multi-turn native session has been validated on this production account.
...
[6] [Native-audio function-calling disconnect investigation](https://zenn.dev/yudai_uk/articles/gemini-native-audio-function-calling-1008-fix?locale=en)

## Release validation — tool-free native retest

The deterministic release gate must pass again after the tool-free native-audio repair: the JavaScript regressions, prescribed Python suite, syntax/import audit, frontend audit, production build, and immutable-asset verification. An authenticated production call must prove that the native session reaches `Listening`, speaks its greeting, remains open across a second user turn, and does not emit an early provider close. Provider function calls remain intentionally out of scope for this validation.
